...
select "Title" from "Album" a order by "Title" asc
...
select "First_Name", "EmployeeId" from "Employee" e 
order by "EmployeeId"  desc 

-- Ejercicio 6:  Muestra el número de géneros distintos disponibles en la base de datos.
select count(distinct "GenreId") from "Track" t 
...
-- Ejercicio 11: Obtén la cantidad de pistas en cada tipo de medio (MediaType) que tienen una duración mayor a 250,000 milisegundos.
 select "MediaTypeId" from "Track" t where "Milliseconds" > 250000 group by "MediaTypeId" 
...
select "BillingPostalCode", count("InvoiceId") from "Invoice" i group by "BillingPostalCode" having count("InvoiceId") > 5
